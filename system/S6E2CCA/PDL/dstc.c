...
/* Change history:
 *
 * ter, Feb 07, 2019: 	Dstc_Init: #if / #def for I2S RX / TX isr handler callback added
 *
 */
...
en_result_t Dstc_Init( const stc_dstc_config_t* pstcConfig )
{
  ...
  stcDstcInternData.pfnDstcI2s0TxCallback = pstcConfig->pfnDstcI2s0TxCallback;
#endif
#if (PDL_ON == PDL_DSTC_ENABLE_I2S0_RX)
  stcDstcInternData.pfnDstcI2s0RxCallback = pstcConfig->pfnDstcI2s0RxCallback;
#endif
  ...
  if ((NULL != pstcConfig->pfnDstcI2s0TxCallback) ||
        (NULL != pstcConfig->pfnDstcI2s0RxCallback))
    {
      if (TRUE == pstcConfig->bTouchNvic)
      {
        #if (PDL_MCU_CORE == PDL_FM4_CORE)
          NVIC_ClearPendingIRQ(DSTC_HW_IRQn);
          NVIC_EnableIRQ(DSTC_HW_IRQn);
          NVIC_SetPriority(DSTC_HW_IRQn, PDL_IRQ_LEVEL_DSTC);
		#endif
      }
    }
...
void Dstc_I2sIrqHandler(uint8_t u8IrqChannel)
{
    ...
    {
        #if (PDL_ON == PDL_DSTC_ENABLE_I2S0_TX)
        	case DSTC_IRQ_NUMBER_I2S0_TX: pfnCallback = stcDstcInternData.pfnDstcI2s0TxCallback;
            break;
        #endif

        #if (PDL_ON == PDL_DSTC_ENABLE_I2S0_RX)
        	case DSTC_IRQ_NUMBER_I2S0_RX: pfnCallback = stcDstcInternData.pfnDstcI2s0RxCallback;
            break;
        #endif
